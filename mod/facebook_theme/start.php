...
//	elgg_register_ajax_view('file/composer');
	...
	if (elgg_is_logged_in() && $_SERVER['REQUEST_URI']=="/nexu/") {
		forward('/dashboard');
	}
...
function facebook_theme_pagesetup_handler() {
	...
	if (elgg_is_logged_in()) {
		...
		elgg_register_menu_item('page', array(
			'name' => 'friends',
			'text' => elgg_view_icon('user') . elgg_echo('friends'),
		...
		elgg_register_menu_item('page', array(
			'name' => 'members',
			'text' => elgg_view_icon('user') . elgg_echo('members'),
			'href' => "/members",
			'priority' => 500,
			'contexts' => array('dashboard'),
		));
		...
		if (elgg_is_active_plugin('groups')) {
			$group_options = array(
				"type" => "group", 
				"full_view" => false,
				"metadata_name_value_pairs" => array("name" => "membership","value" => ACCESS_PUBLIC,"operand" => "<>"),
			);

			$closedGroups = elgg_get_entities_from_metadata($group_options);			
			elgg_register_menu_item('page', array(
				'section' => 'groups',
				'name' => "closedgroupsname",
				'style' => "font-weight:bold;text-transform:capitalize",
				'text' => elgg_echo('groups:closeds'),
			...
			$group_options = array(
				"type" => "group", 
				"full_view" => false,
				"metadata_name_value_pairs" => array("name" => "membership","value" => ACCESS_PUBLIC,),
			);

			$openGroups = elgg_get_entities_from_metadata($group_options);	
			
			elgg_register_menu_item('page', array(
				'section' => 'groups',
				'name' => "opengroupsname",
				'style' => "font-weight:bold;text-transform:capitalize",
				'text' => elgg_echo('groups:opens'),
		...
		if (elgg_is_active_plugin('thewire')) {
			elgg_register_menu_item('page', array(
				'section' => 'more',
				'name' => 'thewire',
				'text' => elgg_view_icon('speech-bubble') . elgg_echo("composer:object:thewire"),
	...
	//		'text' => "<h1 id=\"facebook-topbar-logo\">$site->name</h1>",
			'text' => "<img src='/nexu/_graphics/nexulogo.png' style='margin-top:-7px;margin-left:4px;'/>",
		...
		elgg_register_menu_item('topbar', array(
			...
			'href' => "/blog/view/1766/nexu-tiquette",
			'title' => 'Governance',
			'priority' => 1,
		));
		...
		if (elgg_is_active_plugin('profile')) {
			...
			elgg_register_menu_item('topbar', array(
				'name' => 'profile',
				'section' => 'alt',
				'text' => "<img style=\"padding:0px;\" src=\"{$user->getIconURL('topbar')}\" class=\"elgg-icon elgg-inline-block\" alt=\"$user->name\"/>" . $user->name,
		...
		elgg_register_menu_item('topbar', array(
			'href' => "https://docs.google.com/forms/d/1_lGqbwUfxKEhba_UZZMbKsxVN_e9aYPwddIBn986RX4/viewform?entry.1234166853=http://10.24.5.144".$_SERVER['REQUEST_URI'],
			'name' => 'bugreporting',
			'parent_name' => 'account',
			'priority' => 999, //want this to be at the bottom of the list no matter what
			'section' => 'alt',
			'text' => elgg_echo('admin:plugins:label:bugtracker'),
			'target' => "_blank",
		));
...
function facebook_theme_user_connections_handler() {
	require_once dirname(__FILE__) . '/pages/user_connections.php';
	return true;
}
...
function facebook_theme_composer_menu_handler($hook, $type, $items, $params) {
	$entity = $params['entity'];
	if(!strncmp($_SERVER['REQUEST_URI'],"/nexu/dashboard",strlen("/nexu/dashboard"))) {
		...
		if (elgg_is_active_plugin('blog') && $entity->canWriteToContainer(0, 'object', 'blog')) {
			$items[] = ElggMenuItem::factory(array(
				...
				'text' => elgg_view_icon('speech-bubble-alt') . elgg_echo("composer:object:blog"),
				'priority' => 600,
			));
...
function facebook_theme_river_menu_handler($hook, $type, $items, $params) {
	...
	if (!elgg_in_context('widgets') && !$item->annotation_id && $object instanceof ElggEntity) {
		...
		if ($object instanceof ElggUser && $object->guid != $user->guid && !$object->isFriend()) {
			$items[] = ElggMenuItem::factory(array(
				'name' => 'addfriend',
